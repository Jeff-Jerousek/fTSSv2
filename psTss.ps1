...
Component options:
	...
	-DAcli           # collect DirectAccess client info
...
[CmdletBinding()]param(
	...
	[switch]$DAcli		= $false,					# collect DirectAccess client info
...
function CreateFolder { 
    ...
    if (-not (Test-Path $FolderPath) ){ # if the folder does not already exist
        ...
        Catch{ # since ErrorActionPreference is on 'Stop' we jump into the catch block if New-Item failed 
            ...
            Start-Sleep 2; ExitWithCode -exitcode 1 1           
        }        
    }   
...
# SYNOPSIS: port exhaustion detection: Gets the number of ports in use per process
    ...
    if ($isNetTcpFnd)
    {
        ...
        if (!$isHandleFnd)
        {
            ...
            Start-Sleep 1; ExitWithCode -exitcode 1 1
        }
...
# SYNOPSIS: collect ProcDumps, Example: GetProcDumps "lsass.exe -mp -n 2 -s 5 -AcceptEula $dataPath"
	...
	if (!(Test-Path "$Script:ScriptPath\procdump.exe")) {
		...
		if (!$isDl)
		{
			...
			Start-Sleep 8; ExitWithCode -exitcode 1 1
		}
	}
...
function CheckAdminPrivileges {
    ...
    if (-not $SkipAdminCheck) {
        ...
        if (-not $isAdmin) {
            throw "CheckAdminPrivileges: You do not have the required permission to complete this task. Please run this command in an Administrator PowerShell window or specify the -SkipAdminCheck option."
        }
    }
...
function NormalizeWorkDir {
    ...
    $baseDir = if (-not [String]::IsNullOrWhiteSpace($OutputDirectory)) {
                   if (Test-Path $OutputDirectory) {
                       ...
                       throw "NormalizeWorkDir: The directory ""$OutputDirectory"" does not exist."
                   }
...
function EnvCreate {
    ...
    try {
        ...
        throw "EnvCreate: Failed to create directory ""$OutDir"" because " + $error[0]
    }
} # EnvCreate()
...
function Completion {
    ...
    Param(
        [parameter(Mandatory=$true)] [String] $Src,
		[parameter(Mandatory=$false)] [String] $TssComponent
    )
...
function Get-NetView {
    ...
    Completion -Src $workDir -TssComponent Get-NetView
} #end Get-NetView
#Get-NetView @PSBoundParameters # Entry Point
#endregion ::::: SDN FUNCTIONS :::::
#endregion ::::: SDN  Main Program

#region Get-DAcliLogs ###
function DAcliDetails {
# SYNOPSIS:  generate the same information that the DCA generates for DirectAccess
    [CmdletBinding()]
    Param(
        [parameter(Mandatory=$true)] [String] $OutDir
    )

    $dir = (Join-Path -Path $OutDir -ChildPath "DirectAccess")
    New-Item -ItemType directory -Path $dir | Out-Null

    $file = "DAcliDetails.txt"
	$out = Join-Path $dir $file
    #[String []] $cmds = "netsh lan show interface",
    #                    "netsh lan show profile"
    #ExecCommandsAsync -OutDir $dir -File $file -Commands $cmds
	...
	$user = whoami
	write-output "Direct Access connectivity status for user: $user is" | Out-File -Encoding ascii $out
	$date = date
	Write-output "DATE: $date" | Out-File -Encoding ascii -Append $out
	...
	if($httpprobe -gt 0)
		{
		write-output "=============HTTP PROBES=============" | Out-File -Encoding ascii -Append $out
		foreach ($URL in $httpprobe)
			{
				...
				if($result = 'OK' -and !$test)
					{	write-output "$url Pass" | Out-File -Encoding ascii -Append $out}
				Elseif ($test -match "Unable to connect to the remote server" )
					{	write-output "$url (NAME Resolved)" | Out-File -Encoding ascii -Append $out}
				else 
					{	write-output "$url Failed" | Out-File -Encoding ascii -Append $out}
			}
		}
	else		{	Write-output "There are no HTTP probes configured" | Out-File -Encoding ascii -Append $out}	

	#check if each ICMP probe is accessible
	if($icmpprobe -gt 0)
		{
		write-output "=============ICMP PROBES=============" | Out-File -Encoding ascii -Append $out
		foreach($ip in $icmpprobe)
			{
				$result = ping $ip -n 1
				if($result -match "Packets: Sent = 1, Received = 1, Lost = 0")
					{	write-output "$ip PASS" | Out-File -Encoding ascii -Append $out	}
				elseif($result -match "Pinging")
					{	write-output "$ip Name resolved But ping failed" | Out-File -Encoding ascii -Append $out}
				else
					{ write-output "$ip Failed to resolve name" | Out-File -Encoding ascii -Append $out	}
			}
		}
	else 
		{	Write-output "There are no ICMP probes configured" | Out-File -Encoding ascii -Append $out	}

	#check if DTEs are pingable
	write-output "=============DTES=============" | Out-File -Encoding ascii -Append $out
	if($DTEs) {
	 foreach($ip in $DTEs)
		{
			$result = ping $ip -n 1
			if($result -match "Packets: Sent = 1, Received = 1, Lost = 0")
				{	write-output "DTE: $ip PASS" | Out-File -Encoding ascii -Append $out}
			else
				{	write-output "DTE: $ip Fail" | Out-File -Encoding ascii -Append $out}
		}
	}
	else
				{	write-output "There are no DTE's to test configured in `n HKLM\SOFTWARE\Policies\Microsoft\Windows\NetworkConnectivityAssistant\DTEs " | Out-File -Encoding ascii -Append $out}

	$file = "DAcliDetailsNet.txt"
		[String []] $cmds = "Get-NetIPConfiguration -All -Detailed",
							"systeminfo",
							...
							"Get-NetPrefixPolicy",
							"certutil -silent -store -user my",
							"whoami /all"
		ExecCommands -OutDir $dir -File $file -Commands $cmds 

	#Write-output "END" | Out-File -Encoding ascii -Append $out	
} # end DAcliDetails

function Get-DAcliLogs {
    [CmdletBinding()]
    Param(
        [parameter(Mandatory=$false)]
        [ValidateScript({Test-Path $_ -PathType Container})]
    ...
    $start = Get-Date
    $version = "2018.12.11.0" # Version within date context

    # Input Validation
    #CheckAdminPrivileges $SkipAdminCheck
    $workDir = NormalizeWorkDir -OutputDirectory $OutputDirectory

	Write-Host "Collecting DirectAccess client Diagnostics Data:" -ForegroundColor Green
    Initialization -OutDir $workDir -ExecInMain ($MaxThreads -eq 1)
    ...
    Completion -Src $workDir -TssComponent Get-DAcliLogs
} #end Get-DAcliLogs

#endregion ::::: DAcli FUNCTIONS :::::

#region ::::: CONSTANTS AND VARIABLES :::::
Write-Verbose "Nr. of Args: $($Args.Count) - Nr boundparam: $($psboundparameters.count) - sum: $($psboundparameters.count + $args.count) "
if ($($psboundparameters.count) -eq 0) {Write-Host "Please invoke script with proper arguments."; Show-help; ExitWithCode -exitcode 1 1}
...
If ($RunningAsAdmin -eq $False){
    ...
    Start-Sleep 5; ExitWithCode -exitcode 1 1 # Exit the script now as it doesn´t make sense to run this script in non privileged mode
}

if ( -not ($WorkFolders -or $Ports -or $BindWatch -or $NetView -or $DAcli)) {
	if ($ProviderName -eq '' -or $ProviderName -eq $null) {Write-Log "`n$(Get-Date -Format "HH:mm:ss") === Validation: missing ProviderName, please specify -Trace or any other Component option ===" -tee -foreColor Red
		Start-Sleep 5; ExitWithCode -exitcode 1 1}
	}
...
if ($DAcli) {Get-DAcliLogs -OutputDirectory $dataPath -MaxThreads 5}
